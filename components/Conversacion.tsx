      ...
      <div ref={scrollRef} className="flex-1 space-y-2 overflow-y-auto bg-slate-50 p-4">
        ...
        {messages.map((m) => (
          <div key={m.id} className={`flex ${m.fromMe ? "justify-end" : "justify-start"}`}>
            ...
            >
              {m.text}
              <div className={`mt-0.5 text-[10px] ${m.fromMe ? "text-emerald-100" : "text-slate-400"}`}>
                {m.timestamp
      ...
      <div className="border-t border-slate-100 p-2">
        ...
        <div className="flex items-end gap-2">
          <textarea
            ...
            className="max-h-28 flex-1 resize-none rounded-lg border border-slate-200 px-3 py-2 text-sm outline-none focus:border-slate-400"
          />
          <button
            onClick={enviar}
